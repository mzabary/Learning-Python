...
class PhoneBook:
    ...
    def execute_db_query(self, query, parameters=()):
        try:
            ...
            # with sqlite3.connect(self.db_filename) as conn:
            #    cursor = conn.cursor()
            #    query =  'CREATE TABLE contacts (contactid INTEGER PRIMARY KEY AUTOINCREMENT, name STRING NOT NULL, contactnumber INTEGER NOT NULL );'
            #    query_result = cursor.execute(query, parameters)
            #    conn.commit()
            # return query_result
    ...
    def create_label_frame(self):
        ...
        Label(labelframe, text='Contact Number:').grid(row=2, column=1, sticky=W, pady=2)
        self.numfield = Entry(labelframe)
        self.numfield.grid(row=2, column=2, sticky=W, padx=5, pady=2)
        ttk.Button(labelframe, text='Add Record', command=self.on_add_record_button_clicked).grid(row=3, column=2,
                                                                                                  sticky=E, padx=5,
                                                                                                  pady=2)
    ...
    def create_bottom_buttons(self):
        ttk.Button(text='Delete Selected', command=self.on_delete_selected_button_clicked).grid(row=5, column=0,
                                                                                                sticky=W)
        ttk.Button(text='Modify Selected', command=self.on_modify_selected_button_clicked).grid(row=5, column=1,
                                                                                                sticky=W)
    ...
    def add_new_record(self):
        if self.new_records_validated():
            ...
            self.message['text'] = 'Phone record of {} added'.format(self.namefield.get())
    ...
    def open_modify_window(self):
        ...
        Entry(self.transient, textvariable=StringVar(self.transient, value=old_phone_number), state='readonly').grid(
            row=1, column=2)
        ...
        Button(self.transient, text='Update Record',
               command=lambda: self.update_record(new_phone_number_entry_widget.get(), old_phone_number, name)).grid(
...
#: http: // dejavuwing.tistory.com / entry / Tkinter - SQLite - phonebook - sample[딸구씨]