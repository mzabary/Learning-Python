...
class ForestHouse:
    ...
    def on_contats_button_clicked(self):
        import subprocess
        subprocess.call(["python", "contacts.py"])

    def on_activities_button_clicked(self):
        import subprocess
        subprocess.call(["python", "api2.py"])
    ...
    def del_xls_row(self):
        ...
        try:
            ws2 = wb.get_sheet_by_name('Sheetx')
        except Exception as e:
            ws2 = wb.create_sheet("Sheetx")
            ws2 = wb.get_sheet_by_name('Sheetx')
        ...
        #self.guestfield = self.xlstree.item(item)['values'][0]
        ...
        for rowNum in range(1, ws1.max_row+1):
            ...
            for colNum in range(1, ws1.max_column + 1):
                ...
                if passrow == False:
                    ws2.cell(row=rowNum-skiprow, column=colNum).value = ws1.cell(row=rowNum, column=colNum).value
        #wb.save('airbnb.xlsx')

        #wb = openpyxl.load_workbook("airbnb.xlsx")
        #ss_sheet = wb.get_sheet_by_name('Sheet1')
        ws1.title = 'Sheety'

        #ss_sheet = wb.get_sheet_by_name('Sheetx')
        ws2.title = 'Sheet1'

        #std = wb.get_sheet_by_name('Sheetx')
        #wb.remove_sheet(std)
        std = wb.get_sheet_by_name('Sheety')
        wb.remove_sheet(std)
    ...
    def on_print_xls_selected_button_clicked(self):
        ...
        hDC.StartPage()
        item = self.tree.selection()[0]  # which row did you click on
        multi_line_string=(guestfield+'  '+str(hostdatefield)+'  '+str(nitsfield)+'  '+str(pricefield)+'  '+
    ...
    def create_bottom_buttons(self):
        ...
        ttk.Button(text='Modify Selected',
                   command=self.on_modify_selected_button_clicked).grid(row=5, column=0, sticky=W, padx=200,pady=5)  #W200
        ttk.Button(text='Delete Selected',
                   command=self.on_delete_selected_button_clicked).grid(row=5, column=0, sticky=E,padx=200,pady=5)  #E200
