...
class ForestHouse:
    ...
    def create_label_frame(self):
        ...
        self.namefield.get().grid(row=1, column=2, sticky=W, padx=5, pady=2)
    ...
    def add_record_xls_label_frame(self):
        ...
        parameters = (self.guestfield.get(), self.hostdatefield,
    ...
    def open_modify_xls_window(self):
        print('line 454')
        #style = ttk.Style()
        #guestfield=StringVar()
        #item=self.xlstree.item(self.xlstree.selection())  ##['values'][0]
        ...
        print('Guest:',guestfield)
        self.hostdatefield = self.xlstree.item(self.xlstree.selection())['values'][1]
        print('HostDate:',self.hostdatefield)
        self.nitsfield = self.xlstree.item(self.xlstree.selection())['values'][2]
        print('Nites:',self.nitsfield)
        self.pricefield = self.xlstree.item(self.xlstree.selection())['values'][3]
        print('Price:',self.pricefield)
        self.cleaningfield = self.xlstree.item(self.xlstree.selection())['values'][4]
        print('Clean:',self.cleaningfield)
        self.feefield = self.xlstree.item(self.xlstree.selection())['values'][5]
        print('Fee:',self.feefield)
        self.paidfield = self.xlstree.item(self.xlstree.selection())['values'][6]
        print('paid:',self.paidfield)
        self.commentfield = self.xlstree.item(self.xlstree.selection())['values'][7]
        print('comment:',self.commentfield)
        ...
        guestfield = Entry(labelframe1, width=45)
        guestfield.grid(row=1, column=2, sticky=W, columnspan=3, padx=5, pady=2)
        '''
        Label(labelframe1, text='Host Date:', relief=RIDGE, width=15).grid(row=2, column=1, sticky=W, padx=10, pady=2)
        self.hostdatefield = Entry(labelframe1, width=25)
        self.hostdatefield.grid(row=2, column=2, sticky=W, columnspan=3, padx=5, pady=2)

        Label(labelframe1, text='# Nights:', relief=RIDGE, width=8).grid(row=2, column=3, sticky=E, padx=3, pady=2)
        self.nitsfield = Entry(labelframe1, width=8)
        self.nitsfield.grid(row=2, column=4, sticky=E, padx=5, pady=2)

        Label(labelframe1, text='Price:', relief=RIDGE, width=15).grid(row=3, column=1, sticky=W, padx=10, pady=2)
        self.pricefield = Entry(labelframe1, width=15)
        self.pricefield.grid(row=3, column=2, sticky=W, padx=5, pady=2)

        Label(labelframe1, text='Cleaning:', relief=RIDGE, width=8).grid(row=3, column=3, sticky=E, padx=3, pady=2)
        self.cleaningfield = Entry(labelframe1, width=8)
        self.cleaningfield.grid(row=3, column=4, sticky=E, padx=5, pady=2)

        Label(labelframe1, text='Airbnb Fee:', relief=RIDGE, width=15).grid(row=4, column=1, sticky=W, padx=10, pady=2)
        self.feefield = Entry(labelframe1, width=15)
        self.feefield.grid(row=4, column=2, sticky=W, padx=5, pady=2)

        Label(labelframe1, text='Paid:', relief=RIDGE, width=8).grid(row=4, column=3, sticky=E, padx=3, pady=2)
        self.paidfield = Entry(labelframe1, width=8)
        self.paidfield.grid(row=4, column=4, sticky=E, padx=5, pady=2)

        ttk.Button(labelframe1, text='Save', command=self.on_add_xls_record_button_clicked).grid(
            row=5, column=1, sticky=W, padx=11, pady=2)

        ttk.Button(labelframe1, text='Exit', command=lambda: labelframe1.grid_remove()).grid(
            row=5, column=2, sticky=W, padx=3, pady=2)
        '''

        ee1 = Entry(labelframe1, width=45)
        ee1.insert(45, guestfield)
        ee1.grid(row=1, column=2, sticky=W, columnspan=3, padx=5, pady=2)
        parameters = ee1.get()
        print(parameters)

        """
        e2 = Entry(labelframe1, width=25)
        e3 = Entry(labelframe1, width=8)
        e4 = Entry(labelframe1, width=15)
        e5 = Entry(labelframe1, width=8)
        e6 = Entry(labelframe1, width=15)
        e7 = Entry(labelframe1, width=8)
        #e8 = Entry(labelframe1, width=30)

        e1.insert(45, self.guestfield)
        e2.insert(25, self.hostdatefield)
        e3.insert(8, self.nitsfield)
        e4.insert(15, self.pricefield)
        e5.insert(8, self.cleaningfield)
        e6.insert(15, self.feefield)
        e7.insert(8, self.paidfield)
        #e8.insert(30, self.commentfield)

        e1.grid(row=1, column=2, sticky=W, columnspan=3, padx=5, pady=2)
        e2.grid(row=2, column=2, sticky=W, columnspan=3, padx=5, pady=2)
        e3.grid(row=2, column=4, sticky=E, padx=5, pady=2)
        e4.grid(row=3, column=2, sticky=W, padx=5, pady=2)
        e5.grid(row=3, column=4, sticky=E, padx=5, pady=2)
        e6.grid(row=4, column=2, sticky=W, padx=5, pady=2)
        e7.grid(row=4, column=4, sticky=E, padx=5, pady=2)
        #e8.grid(row=3, column=1, padx=10, pady=5)

        #Button(win, text='Quit', command=win.destroy, fg='green').grid(row=4, column=1, sticky=W,
        #  pady=4, padx=10)
        #Button(win, text='Update', fg='red', command=lambda: self.modify_record(e1.get(),
         e2.get(), e3.get(), e4.get())).grid(row=4, column=1, sticky=E, pady=4, padx=10)

        parameters=(e1.get(),e2.get(),e3.get(),e4.get(),e5.get(),e6.get(),e7.get())
        #  sticky=E, pady=4, padx=10)
        self.message['text'] = '                                     '
        self.create_left_icon
    ...
    def create_xls_bottom_buttons(self):
        ...
        ttk.Button(text='Modify Selected',
                   command=self.on_modify_xls_selected_button_clicked).grid(row=5, column=0, sticky=W, padx=150)
        ttk.Button(text='Delete Selected',
